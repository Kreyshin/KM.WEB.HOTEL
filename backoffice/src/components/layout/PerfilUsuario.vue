...
<template>
  <div ref="contenedor" class="relative mt-auto">
    ...
    <div
      v-if="abierto"
      class="hs-entrada absolute bottom-0 left-full z-40 ml-3 w-64 overflow-hidden rounded-overlay border border-linea bg-panel"
    ...
    >
      ...
      >
        ...
        <span
          ...
          :class="ui.tema === 'oscuro' ? 'bg-azul' : 'bg-linea'"
        >
          <span
      ...
      <button
        type="button"
        class="w-full border-t border-linea px-4 py-3 text-left text-sm font-medium text-coral transition-colors hover:bg-coral/10 dark:text-[var(--hs-coral-claro)]"
        @click="cerrarSesion"
      >
        Cerrar sesión
      </button>
