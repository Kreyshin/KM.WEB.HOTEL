...
<template>
  <div
    ref="raiz"
    class="hs-hora"
    :class="{ 'hs-hora-invalida': invalido, 'hs-hora-abierta': abierto, 'opacity-50': disabled }"
  >
    <input
      :id="id"
      v-model="texto"
    ...
    <Teleport to="body">
      <div
        ...
        @keydown.esc="cerrar"
      >
        <div class="hs-hora-cabecera"><span>Hora</span><span>Min</span></div>
        <div class="flex gap-1">
          <ul role="listbox" aria-label="Horas" class="hs-hora-columna">
            <li v-for="h in horas" :key="h">
              <button
                type="button"
                role="option"
                :aria-selected="h === hh"
                class="hs-hora-opcion"
                @click="elegir('h', h)"
              >
                {{ h }}
              </button>
          ...
          <ul role="listbox" aria-label="Minutos" class="hs-hora-columna">
...
.hs-hora-opcion:hover {
  background: var(--color-seleccion);
}
.hs-hora-opcion[aria-selected='true'] {
