...
<template>
  ...
  >
    ...
    <button
      ...
      :aria-label="etiqueta"
      :disabled="disabled"
      class="hs-switch relative mt-0.5 h-6 w-11 shrink-0 rounded-full"
      :class="activo ? 'bg-accion' : 'bg-linea'"
      @click="activo = !activo"
    >
      <span
        class="hs-switch-tirador absolute top-0.5 left-0.5 size-5 rounded-full bg-white shadow-sm"
...
.hs-switch:active:not(:disabled) .hs-switch-tirador {
  width: 1.625rem;
}

.hs-switch:active:not(:disabled) .hs-switch-tirador.translate-x-5 {
