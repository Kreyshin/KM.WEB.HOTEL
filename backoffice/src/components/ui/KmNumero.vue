 ...
 */
const botonControl =
  'hs-paso grid shrink-0 place-items-center text-tenue disabled:pointer-events-none disabled:opacity-30'
</script>

<template>
  <div
    class="hs-campo hs-numero flex h-[var(--km-toque,2.5rem)] w-full items-stretch overflow-hidden rounded-control border bg-panel text-[length:var(--km-texto,0.875rem)]"
    :class="[
  ...
  >
    ...
    <input
      ...
      :aria-valuemin="min"
      :aria-valuemax="max"
      class="hs-sin-anillo min-w-0 flex-1 bg-transparent px-2 font-semibold text-tinta tabular-nums outline-none placeholder:font-normal placeholder:text-tenue/70"
...
.hs-paso:active:not(:disabled) svg {
