...
<template>
  <label
    class="hs-check inline-flex cursor-pointer items-start select-none"
    :class="[
  ...
  >
    ...
    <span class="hs-check-caja" :class="tamano === 'sm' ? 'size-4' : 'size-5'" aria-hidden="true">
...
input:checked + .hs-check-caja {
  background: var(--color-accion);
  border-color: var(--color-accion);
  transform: scale(1);
  animation: hs-check-pellizco var(--km-mov-normal) var(--km-rebote);
}

input:checked + .hs-check-caja svg {
  stroke-dashoffset: 0;
}

input:focus-visible + .hs-check-caja {
  box-shadow: 0 0 0 3px color-mix(in srgb, var(--color-accion) 30%, transparent);
}

@keyframes hs-check-pellizco {
  0% {
