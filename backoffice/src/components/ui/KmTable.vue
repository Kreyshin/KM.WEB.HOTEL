      ...
      <thead>
        <tr class="border-b border-linea">
          <th
            ...
            :key="c.clave"
            scope="col"
            class="hs-etiqueta px-4 py-3 text-left text-tenue"
            :class="c.clase"
          ...
          >
            <button
              v-if="c.ordenable"
              type="button"
              class="hs-etiqueta -mx-1 inline-flex items-center gap-1 rounded px-1 py-0.5 transition-colors hover:text-tinta"
        ...
        <tr v-if="error">
          <td :colspan="columnas.length">
            <KmEstado tipo="error" titulo="No se pudieron cargar los datos" :mensaje="error">
              <button
                type="button"
                class="text-sm font-semibold text-azul hover:underline"
